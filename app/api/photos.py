...
Файлы кладутся в папку UPLOAD_DIR (settings.UPLOAD_DIR). На проде (Railway)
это /data/uploads — подключён постоянный Volume к сервису pacific-analysis
(смонтирован на /data), так что файлы переживают передеплой. Локально без
переменной окружения UPLOAD_DIR падает обратно на ./uploads (эфемерно, но
для локальной разработки это не важно). Если понадобится S3/Cloudinary —
